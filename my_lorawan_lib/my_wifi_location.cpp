...
void My_Wifi_Location::prepare_wifi_msg(uint8_t *msg_buffer, uint8_t &msg_size)
{
    ...
    msg_size = 0;
    // assuming max message size of 115 -> 14 networks
    // max number of networks that fit a msg is 14, so we cannot send more than that
    for (int i = 0; i < num_networks && i < 14; ++i)
    {
        uint8_t array[8];
